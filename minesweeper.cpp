...
void render() {
    ...
    const bool neg = (flags < 0);

    int absFlags = std::abs(flags);

    int digs[] = {
        absFlags / 100 % 10, // Hundreds place
        absFlags / 10 % 10,  // Tens place
        absFlags % 10        // Units place
    };
    ...
    for (int i = 0; i < 3; i++) {
        sps[i].setTextureRect(rects[(i == 0 && neg) ? 10 : digs[i]]);
    }

    // Position the sprites
    sps[0].setPosition(0, 512);
    sps[1].setPosition(21, 512);
    sps[2].setPosition(42, 512);

    for (auto& s : sps)t.window.draw(s);

    sf::Sprite* btnSprites[] = {new sf::Sprite(t.debug), new sf::Sprite(t.test1), new sf::Sprite(t.test2)};
    ...
    t.newGameButton->setSprite(new sf::Sprite(
        t.gameState->getPlayStatus() == GameState::PLAYING ? t.happy :
        (t.gameState->getPlayStatus() == GameState::WIN ? t.win : t.lose)));

    // Set button positions
    Button* btns[] = {t.debugButton, t.newGameButton, t.testButton1, t.testButton2};
    for (auto* b : btns) b->getSprite()->setPosition(b->getPosition().x, b->getPosition().y);
    for (auto* b : btns) t.window.draw(*b->getSprite());
    
    // Draw the game board
    for (int j = 0; j < t.board.size(); j++) {
        for (int k = 0; k < t.board[0].size(); k++) {
            t.gameState->getTile(k, j)->draw();
        }
    }
}
